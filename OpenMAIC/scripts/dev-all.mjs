...
function shouldIgnoreServiceExit(serviceName, code) {
  if (serviceName !== 'openmaic' || code === 0) {
    return false;
  }
...
async function main() {
  ...
  if (fs.existsSync(nextDevLockPath)) {
    process.stdout.write('[dev-all] Detected existing Next dev lock, reusing the running OpenMAIC dev server.\n');
    ignoredExitServices.add('openmaic');
  } else {
    services.unshift({
      name: 'openmaic',
      cwd: rootDir,
